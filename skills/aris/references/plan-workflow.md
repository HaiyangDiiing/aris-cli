# Plan Workflow — /aris:plan

Convert a textual goal into a validated, ready-to-execute ARIS configuration.

**Output:** A complete `/aris` invocation with Scope, Metric, Direction, and Verify --
all validated before launch.

## Trigger

- User invokes `/aris:plan`
- User says "help me set up aris", "plan an experiment run", "what should my metric be"

## Workflow

### Phase 1: Capture Goal

**CRITICAL -- BLOCKING PREREQUISITE:** If no goal is provided inline, you MUST use
`AskUserQuestion` to capture it. DO NOT skip this step or proceed to Phase 2 without
a goal.
...
1. Read codebase structure (package.json, Cargo.toml, pyproject.toml, etc.)
...
```
AskUserQuestion:
  question: "Which files should ARIS be allowed to modify?"
  header: "Scope"
  options:
...
This is the critical step. The metric must be **mechanical** -- extractable from a
command output as a single number.
...
| Check | Pass | Fail |
|-------|------|------|
| Outputs a number | `87.3`, `0.95`, `42` | `PASS`, `looks good`, checkmark |
...
**Do not proceed until metric is mechanical.**
...
```
AskUserQuestion:
  question: "Do you want a guard command? This prevents breaking existing behavior."
  header: "Guard"
  options:
    ...
    - label: "Yes -- line count guard to prevent bloat"
      description: "Reject changes that grow total lines beyond baseline + 10%"
    - label: "No guard needed"
      description: "Skip -- the metric is enough"
```

**Guard suggestion rules:**
- If metric is performance/benchmark/bundle size -> suggest test command as guard
- If metric is Lighthouse/accessibility -> suggest test command as guard
- If metric is refactoring (LOC reduction) -> suggest test + typecheck as guard
...
**If metric-valued guard chosen:** Collect direction and threshold:
```
AskUserQuestion:
...
**Guard validation:** Run guard once to confirm it passes on current codebase. If it
fails, help user fix it before proceeding.
...
3. **Extract the metric and validate it is a number** -- the final output must match
   `^-?[0-9]+\.?[0-9]*$` (integer or decimal, optional leading minus). Strings like
   "PASS", "85.2%", "342ms", empty output, or multi-line output all fail.
...
| `All files \| 85.2 \| ...` | awk field wrong | Adjust awk field index |
| Two numbers on separate lines | Pipeline too broad | Add `head -1` or tighten grep |

**Do not proceed if verify command fails dry run.** Help user fix the pipeline until it
produces a single valid number.
...
AskUserQuestion:
  ...
  options:
    - label: "Launch now -- unlimited (Recommended)"
      description: "Start /aris immediately, loop until interrupted"
...
If "Launch now -- unlimited": invoke `/aris` with the configuration.
If "Launch now -- bounded": ask for iteration count, then invoke with `Iterations: N`.
